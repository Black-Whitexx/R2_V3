/**
  ...
  * @file           : My_Can.c
  * @author         : 86153
  * @brief          : 触发中断时接受数据并存入对应全局变量数组，供上层调用
  * @attention      : None
  * @date           : 2024/4/27
  ******************************************************************************
  */
...
Can_Frame fdcan1_buffer[Can1Peripheral_NUM];//将can帧整合到一个buffer数组里
Can_Frame fdcan2_buffer[Can2Peripheral_NUM];
...
uint8_t MyCan_SendData(FDCAN_HandleTypeDef *hcan,uint32_t id,uint8_t *data){
    ...
    TxHeader.BitRateSwitch=FDCAN_BRS_OFF;               //关闭速率切换S
...
void HAL_FDCAN_RxFifo0Callback(FDCAN_HandleTypeDef *hfdcan, uint32_t RxFifo0ITs)
{
    ...
    if((RxFifo0ITs & FDCAN_IT_RX_FIFO0_NEW_MESSAGE) != RESET)
    {
        if(hfdcan->Instance == FDCAN1)
        {
            ...
            {
                for (int i = 0; i < Can1Peripheral_NUM; i++) {
                    if (fdcan1_buffer[i].id == RxHeader.Identifier) {
                        copyArray(rxdata,fdcan1_buffer[i].data,8);
                    }
                }
...
//                    //简单数组间copy
//                    for(int i = 0;i < 8;i++){
//                        fdcan1_buffer[index][i] = rxdata[i];
//                    }
//                    //MotorInfo_Record(&DjiMotor_Info[index], rxdata);   //解包
//                }
            }
        ...
        {
            HAL_RetVal = HAL_FDCAN_GetRxMessage(hfdcan, FDCAN_RX_FIFO0, &RxHeader, rxdata);      //从CAN2接收数据，通过过滤器后放入FIFO0,存入RxMessage数据帧
            if(HAL_RetVal == HAL_OK)
            {
                for (int i = 0; i < Can2Peripheral_NUM; i++) {
                    if (fdcan2_buffer[i].id == RxHeader.Identifier) {
                        copyArray(rxdata,fdcan2_buffer[i].data,8);
                    }
                }
...
//                    //简单数组间copy
//                    for(int i = 0;i < 8;i++){
//                        fdcan2_buffer[index][i] = rxdata[i];
//                    }
//                    //MotorInfo_Record(&DjiMotor_Info[index], rxdata);   //解包
//                }
            }
